...
<?php
    ...
    class AgentLogoff
    {
        ...
        public function StasisAppEventHandler()
        {
            $this->stasisEvents->on('StasisStart', function ($event) {
                ...
                $this->stasisLogger->notice("Remove {$event->channel->caller->name} from DB");
		include "db_info.php";
		$db = new mysqli($dbhost, $dbuser, $dbpass, $dbname) or die($db->connect_error);
		$sql = "DELETE FROM agents WHERE exten = '{$event->channel->caller->number}'";
		$db->query($sql) or die($db->error);
                $this->stasisChannelID = $event->channel->id;
                $this->phpariObject->channels()->channel_answer($this->stasisChannelID);
                $this->phpariObject->channels()->channel_playback($this->stasisChannelID, 'sound:agent-loggedoff', NULL, NULL, NULL, 'play1');
            });
    ...
    $basicAriClient = new AgentLogoff("agentLogoff");
